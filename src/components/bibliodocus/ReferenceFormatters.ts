  ...
  const formattedAuthors = authors.map((author) => {
    ...
    if (author.includes(",")) {
      ...
      switch (style) {
        ...
        case "harvard":
          // Harvard: Last, F.
          const harvardInitials = firstName
            .split(" ")
            .map((part) => `${part.charAt(0)}.`)
            .join(" ");
 ...
 * @param style The citation style
 */
export const formatArticleTitle = (
  title: string | undefined,
  style: ReferenceStyle,
): string => {
  if (!title) return "";
...
export const formatReference = (
  ...
  switch (entryType.toLowerCase()) {
    ...
    case "incollection":
    case "conference":
      return formatConferenceReference(
        author,
        title,
        booktitle,
        editor,
