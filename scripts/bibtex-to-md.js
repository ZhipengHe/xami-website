...
console.log(`Overwrite mode: ${OVERWRITE_EXISTING ? 'ON' : 'OFF'} (use --force to enable overwriting)`);
...
bibtexEntries.forEach(entry => {
  try {
    ...
    const title = entryTags.TITLE ? entryTags.TITLE.replace(/\s+/g, ' ').trim() : 'Untitled';
    ...
    if (entryTags.AUTHOR) {
      ...
      authors = authorStr.split(/\s+and\s+/i)
    ...
    if (entryTags.KEYWORDS) {
      const keywords = entryTags.KEYWORDS.split(',').map(k => k.trim().toLowerCase());
      tags.push(...keywords);
    }
...
`;
    ...
    console.error(`Error processing entry ${entry.citationKey || 'unknown'}: ${error.message}`);
  }
});
...
function parseAuthors(authorString) {
  ...
  const normalized = authorString.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
